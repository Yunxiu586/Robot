...
The rows of the matrix ${}^{A}R_B$ are the transposes of the unit vectors of {$A$} expressed in {$B$}.
$$
{}^{A}R_B=\left[ \begin{matrix}
   ({}^{B}X_A)^{\text{T}}  \\
   ({}^{B}Y_A)^{\text{T}}  \\
   ({}^{B}Z_A)^{\text{T}}  \\
\end{matrix} \right]
...
$$
({}^{A}R_B)^{\text{T}}{}^{A}R_B=\left[ \begin{matrix}
   ({}^{A}X_B)^{\text{T}}  \\
   ({}^{A}Y_B)^{\text{T}}  \\
   ({}^{A}Z_B)^{\text{T}}  \\
\end{matrix} \right]\left[ \begin{matrix}
...
is a coordinate system where we give the orientation and a position vector which locates its origin relative to some other embedding frame.
...
**Mapping** means changing descriptions from frame to frame.
...
{^AP_2}={R_K{(\theta)}}{^AP_1}
$$
In general, rotations about different axes do not commute:
$$
R_{k_1}(\alpha)R_{k_2}(\beta)\neq R_{k_2}(\beta)R_{k_1}(\alpha).
$$
...
$$
{^BP_{AORG}}=-{}^{B}R_A{}^{A}P_{BORG}=-({}^{A}R_B)^{\text{T}}{}^{A}P_{BORG}
$$
...
<img src="../../Figures/Spatial_descriptions_and_transformations_transform_equation.png" alt="Spatial_descriptions_and_transformations_transform_equation" style="zoom:66%;" />
...
$$
{}^{B}T_C=({}^{U}T_B)^{-1}{}^{U}T_A{}^{A}T_D({}^{C}T_D)^{-1}
$$
...
$$
These extraction formulas apply when $\cos\beta\neq0$. When $\cos\beta=0$, the representation is singular and the roll and yaw angles cannot be determined independently.
...
Start with the frame coincident with a known frame {$A$}. Rotate {$B$} first about $Z_B$ by an angle $\alpha$, then about $Y_B$ by an angle $\beta$, and finally about the moving $Z_B$ axis by an angle $\gamma$.
$$
...
$$
These extraction formulas apply when $\sin\beta\neq0$. When $\sin\beta=0$, the Z-Y-Z representation is singular and $\alpha$ and $\gamma$ cannot be determined independently.

##### Equivalent angle-axis representation

Let $C$ be a rotation matrix whose third column is the unit rotation axis $K$ expressed in frame {$A$}:
$$
C=\begin{bmatrix}
{n_{x}} & {o_{x}} & {a_{x}} \\
{n_{y}} & {o_{y}} & {a_{y}} \\
...
For an arbitrary vector $b$ expressed in frame {$A$}, define its coordinates in frame {$C$} as $x$:
$$
b=Cx
$$
...
Rotating $b$ about $K$ is equivalent to expressing it in frame {$C$}, rotating it about the $z$ axis, and transforming it back to frame {$A$}:
$$
...
\end{bmatrix}
$$
The expression for $K$ applies when $\sin\theta\neq0$. The cases $\theta=0$ and $\theta=\pi$ require separate treatment.
...
$$
These formulas use the branch $\epsilon_4\neq0$. When $\epsilon_4$ is zero or numerically close to zero, a branch based on the largest diagonal element of the rotation matrix should be used instead.
...
In the case of a free vector, all that counts is the **magnitude** and **direction**. Only the rotation matrix relating the two systems is used in transforming. The relative locations of the origins do not enter into the calculation.
