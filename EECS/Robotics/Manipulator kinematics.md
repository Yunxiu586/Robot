...
<img src="../../Figures/Manipulator_kinematics_link_parameters.png" alt="Manipulator_kinematics_link_parameters" style="zoom:60%;" />
...
For nonparallel joint axes, $X_i$ is chosen along the common normal from $Z_i$ to $Z_{i+1}$. When the axes intersect, a consistent convention is
$$
X_i\parallel Z_i\times Z_{i+1}.
$$
When the axes are parallel, the cross product is zero, so $X_i$ must instead be selected along a common normal.
...
+ Rotate about $X_{i-1}$ by an angle $\alpha_{i-1}$.
+ Translate along $X_{i-1}$ by a distance $a_{i-1}$.
+ Rotate about $Z_i$ by an angle $\theta_i$.
...
+ For a revolute joint, $\theta_i$ is the joint variable and $d_i$ is constant.
+ For a prismatic joint, $d_i$ is the joint variable and $\theta_i$ is constant.
+ The parameters $a_{i-1}$ and $\alpha_{i-1}$ are fixed by the link geometry for both joint types.
...
The forward kinematics focuses on the forward mappings from actuator space to joint space, and from joint space to Cartesian space. The inverse kinematics addresses the inverse mappings.
...
<img src="../../Figures/Manipulator_kinematics_standard_frames.png" alt="Manipulator_kinematics_standard_frames" style="zoom:60%;" />
...
$$
{}^{S}T_T=({}^{B}T_S)^{-1}{}^{B}T_W{}^{W}T_T
$$
<img src="../../Figures/Manipulator_kinematics_where_function.png" alt="Manipulator_kinematics_where_function" style="zoom:70%;" />

### PUMA 560

The Unimation PUMA 560 is a robot with six degrees of freedom and all rotational joints.

<img src="../../Figures/Manipulator_kinematics_puma560.png" alt="Manipulator_kinematics_puma560" style="zoom:70%;" />
...
<img src="../../Figures/Manipulator_kinematics_workspace.jpg" alt="Manipulator_kinematics_workspace" style="zoom:9%;" />
...
<img src="../../Figures/Manipulator_kinematics_planar_robot.jpg" alt="Manipulator_kinematics_planar_robot" style="zoom:8%;" />
...
The right-hand side of the above equation must have a value between -1 and 1. This constraint is checked to determine whether a solution exists. If it is not satisfied, the goal point is either farther than $l_1+l_2$ from the base or closer than $|l_1-l_2|$.
...
Assuming a solution exists, one geometric branch is obtained by choosing $-180^{\circ}\leq\theta_2\leq0^{\circ}$; the other branch corresponds to the opposite elbow configuration.

<img src="../../Figures/Manipulator_kinematics_geometric_solution.jpg" alt="Manipulator_kinematics_geometric_solution" style="zoom:16%;" />
...
$$
Here, the arccosine must be solved so that $0\le\psi\le180^{\circ}$. Then we have
$$
\theta_1=\beta\pm\psi
$$
...
+ If $\sin\alpha_1=0$, then $z=k_4$, where $z$ is known.
...
Having obtained $\theta_1$, $\theta_2$, and $\theta_3$, we can compute the inverse of the orientation of frame {4} relative to frame {0}, evaluated at $\theta_4=0$. Because the problem specifies ${}^{0}R_6$, we can compute
$$
\left.{}^{4}R_6\right|_{\theta_4=0}=\left.({}^{0}R_4)^{-1}\right|_{\theta_4=0}{}^{0}R_6.
$$
...
In a nonsingular configuration, these last three joints generally have two solution branches, so the total number of solutions can be twice the number found for the first three joints. At a wrist singularity, the solutions may instead be degenerate or non-unique.
...
-p_z &= a_{3} \sin(\theta_2+\theta_3) + a_{2} \sin\theta_2 + d_{4} \cos(\theta_2+\theta_3)
\end{align*}
$$
...
\end{bmatrix}
$$
Equating both the (1,3) elements and the (3,3) elements from the two sides of $\left[{ }^{0}T_{4}(\theta_{4})\right]^{-1}{ }^{0}T_{6} = { }^{4}T_{6}$
$$
\begin{aligned}
...
$$
Equating the corresponding rotation-matrix elements from the two sides of $({}^{0}T_{5})^{-1}{}^{0}T_{6}={}^{5}T_{6}$
$$
