...
<img src="../../Figures/ctrl_theory_real_axis_rule.png" alt="Real-axis root-locus rule" style="zoom:50%;" />
...
For a real-axis breakaway or break-in point where $r$ branches meet, the local branch directions are separated by $2\pi/r$. Their absolute orientation is determined from the local expansion of the characteristic equation. For the common double-root case $r=2$, the complex branches leave or approach the real axis at angles $\pm90^\circ$.

<img src="../../Figures/ctrl_theory_break_points.png" alt="Breakaway and break-in points" style="zoom:45%;" />
...
<img src="../../Figures/ctrl_theory_departure_arrival.png" alt="Departure and arrival angles" style="zoom:40%;" />
...
An imaginary-axis crossing places one or more closed-loop poles on the stability boundary. The system is marginally stable only if the imaginary-axis poles are simple and all remaining closed-loop poles lie in the open left-half $s$-plane.
...
The damping ratio is related to the angle $\phi$ measured from the negative real axis.
...
$$
L(s)=k\frac{s+2}{s^2+2s+2}
$$

The open-loop poles and zero are
...
Thus, $n=2$ and $m=1$. There are two root-locus branches. They start at $-1\pm j$; one ends at the finite zero $-2$, and the other goes to infinity. Since the only real open-loop singularity is the zero at $-2$, the real-axis root-locus segment is
...
$$
1+k\frac{s+2}{s^2+2s+2}=0
$$

or

$$
D(s)=s^2+(k+2)s+(2k+2)=0
$$

The departure angle from the upper pole $p_1=-1+j$ is
...
so the closed-loop system is stable for every root-locus gain $k\ge0$, and the locus does not cross the imaginary axis.

<img src="../../Figures/ctrl_theory_circular_locus.png" alt="Circular second-order root locus" style="zoom:40%;" />
...
<img src="../../Figures/ctrl_theory_gain_example.png" alt="Root-locus gain, stability, and damping example" style="zoom:40%;" />
...
The corresponding approximate root-locus gain range is
